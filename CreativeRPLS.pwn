...
Change Notes 1.6.4:
1. [IMPROVE] UCP Register Problem and add VIP Donation Package
2. [CHANGE] Login Information and SendMessageClient(Don't Ask This!) color
3. [ADD] New Bank Mappings
3. [CHANGE] SAMP Slot to 30
4. [CHANGE] Color Message Admin CMD /aa
5. [ADD] Staff Rank Manager

Change Notes 1.6.6:
1. [ADD] Vehicle Lock Hotkey "2"
2. [ADD] Items Hotkey "Y"
3. [ADD] Gate open Hotkey "H / CapsLock"
4. [ADD] Admin teleport information to Client
5. [CHANGE] Animation while injured.
6. [FIX] Bug infinity health after admin duty

Change Notes 1.6.7:
1. [DISABLE] Permanently animation reset hotkeys
2. [CHANGE] ASK system, Delete Admin names on answer to Client
3. [CHANGE] User login information, Change to player spawn not Connect.
4. [FIX] Login dialog mistakes word
5. [ADD] Bad Password slot to 5 from 3
6. [ADD] Admin Command "/gotobank"
7. [ADD] Admin Command "/gotoasgh"

Change Notes 1.6.8u1:
1. [DISABLE] Auto RP while buy something in business.
2. [ADD] Auto '/b' while admin/helper on duty.
3. [CHANGE] Helper duty nametag color to 'RED' from 'GREEN'
4. [CHANGE] Mechanic duty nametag color change to 'LIGHTGREEN' from 'GREEN'
5. [DISABLE] Auto RP while pick weapons on sapd locker.
6. [CHANGE] Vehicle engine hotkey to 'ALT' from 'Y'
...
SetPlayerJoinCamera(playerid)
{
	...
	switch(randcamera)
	{
		...
		case 6: // Market
		{
			SetPlayerPos(playerid, 1134.2405, -1126.4259, 24.4274);
			//TogglePlayerSpectating(playerid, 1);
            TogglePlayerControllable(playerid, 0);
			InterpolateCameraPos(playerid, 809.276245, -1148.044677, 25.782951, 1333.784790, -1149.513061, 27.081487, 60000);
			InterpolateCameraLookAt(playerid, 814.275634, -1148.093750, 25.844146, 1328.788696, -1149.370971, 26.942182, 60000);
		}
		case 7: // Deket SANews
		{
			SetPlayerPos(playerid, 1040.7625,-1383.2595,13.7062);
			//TogglePlayerSpectating(playerid, 1);
			TogglePlayerControllable(playerid, 0);
			InterpolateCameraPos(playerid, 628.496765, -1401.195434, 17.121500, 1265.677856, -1400.795532, 17.793922, 60000);
			InterpolateCameraLookAt(playerid, 633.494934, -1401.098632, 17.028158, 1260.678222, -1400.819335, 17.740680, 60000);
		}
		case 8: // Jalan Tol
		{
			TogglePlayerSpectating(playerid, 1);
			//TogglePlayerControllable(playerid, 0);
			InterpolateCameraPos(playerid, 1589.930664, -1710.862670, 84.797180, 1642.369995, -1086.448486, 108.982246, 60000);
			InterpolateCameraLookAt(playerid, 1590.267700, -1705.881835, 84.517639, 1641.965087, -1091.364624, 108.164840, 60000);
		}
...
new EditingObject[MAX_PLAYERS], EditingObjectID[MAX_PLAYERS];
...
#include "Extras/Color.inc"
#include "Extras/Textdraw.inc"
...
#include "Modules/CMD/Admin.inc"
#include "Modules/CMD/Faction.inc"
#include "Modules/CMD/Player.inc"

#include "Modules/BOOMBOX.pwn"
#include "Modules/QUIZ.pwn"
#include "Modules/VOTE.pwn"
#include "Modules/Actor/Actor.inc"
//#include "Modules/Animation/Anim.inc"
//#include "Modules/Siren/Siren.inc"
#include "Modules/MOTD/MOTDs.inc"
...
public OnGameModeInit()
{
	...
	mysql_tquery(g_SQL, "SELECT * FROM `objects`", "LoadObjects");
	...
	g_Discord_Chat = DCC_FindChannelById("816611763679658025"); // Discord channel ID
	...
	SAPDVehicles[6] = AddStaticVehicleEx(598,1595.5073,-1710.6666,5.6109,0.1711,0,1, VEHICLE_RESPAWN, 1); // Cruiser
	SAPDVehicles[7] = AddStaticVehicleEx(598,1591.3687,-1710.6256,5.6119,358.8773,0,1, VEHICLE_RESPAWN, 1); // Cruiser
	...
	SAPDVehicles[16] = AddStaticVehicleEx(528,1527.6542,-1655.9646,5.9339,270.1992,0,0, VEHICLE_RESPAWN, 1); // SWAT Van
	SAPDVehicles[17] = AddStaticVehicleEx(560,1584.7515,-1667.5281,5.5974,270.7136,0,0, VEHICLE_RESPAWN, 1); // Sultan
	SAPDVehicles[18] = AddStaticVehicleEx(560,1584.7291,-1671.6987,5.5982,268.7675,0,0, VEHICLE_RESPAWN, 1); // Sultan
	SAPDVehicles[19] = AddStaticVehicleEx(523,1584.7783,-1675.3850,5.4639,270.6620,0,0, VEHICLE_RESPAWN, 1); // Police Bike
	SAPDVehicles[20] = AddStaticVehicleEx(523,1585.0814,-1680.4186,5.4661,272.0062,0,0, VEHICLE_RESPAWN, 1); // Police Bike
	SAPDVehicles[21] = AddStaticVehicleEx(523,1587.2833,-1677.7260,5.4654,269.2448,0,0, VEHICLE_RESPAWN, 1); // Police Bike
	SAPDVehicles[22] = AddStaticVehicleEx(596,1535.9803,-1677.6580,13.1176,359.4672,0,1, VEHICLE_RESPAWN, 1); // Cruiser (Front)
	SAPDVehicles[23] = AddStaticVehicleEx(596,1535.9502,-1667.3008,13.1039,180.1666,0,1, VEHICLE_RESPAWN, 1); // Cruiser (Front)
	SAPDVehicles[24] = AddStaticVehicleEx(525,1556.5985,-1606.3214,13.2680,179.1279,17,20, VEHICLE_RESPAWN, 1); // Tow Truck
	SAPDVehicles[25] = AddStaticVehicleEx(525,1560.8201,-1606.3423,13.2557,178.6305,18,20, VEHICLE_RESPAWN, 1); // Tow Truck
	SAPDVehicles[26] = AddStaticVehicleEx(497,1569.1587,-1641.0361,28.5788,89.5537,0,1, VEHICLE_RESPAWN, 1); // Maverick
	SAPDVehicles[27] = AddStaticVehicleEx(497,1547.7992,-1643.6317,28.5923,91.2595,0,1, VEHICLE_RESPAWN, 1); // Maverick
 	SAPDVehicles[28] = AddStaticVehicleEx(411,1578.5643,-1710.6968,5.6112,0.0933,0,1, VEHICLE_RESPAWN, 1); // Infernus
	SAPDVehicles[29] = AddStaticVehicleEx(411,1574.3217,-1710.7924,5.6117,0.8341,0,1, VEHICLE_RESPAWN, 1); // Infernus
	SAPDVehicles[30] = AddStaticVehicleEx(541,1570.2491,-1709.9867,5.8906,0.4471,0,1, VEHICLE_RESPAWN, 1); // Bullet
	SAPDVehicles[31] = AddStaticVehicleEx(451,1558.8839,-1710.6243,5.8906,0.0085,0,1, VEHICLE_RESPAWN, 1); // Turismo
	SAPDVehicles[32] = AddStaticVehicleEx(451,1564.6312,-1709.3994,5.8906,0.2591,0,1, VEHICLE_RESPAWN, 1); // Turismo
	SAPDVehicles[33] = AddStaticVehicleEx(522,1544.2211,-1707.7697,5.8906,335.6310,0,0, VEHICLE_RESPAWN, 1); // NRG-500
	SAPDVehicles[34] = AddStaticVehicleEx(522,1542.7397,-1706.2843,5.8906,318.2722,0,0, VEHICLE_RESPAWN, 1); // NRG-500
	SAPDVehicles[35] = AddStaticVehicleEx(552,1529.6619,-1688.0270,5.8906,270.5197,0,0, VEHICLE_RESPAWN, 1); // Utility Truck
	SAPDVehicles[36] = AddStaticVehicleEx(437,1554.0326,-1625.1271,13.3828,92.2224,1,3, VEHICLE_RESPAWN, 1); // Coach
	SAPDVehicles[37] = AddStaticVehicleEx(490,1545.5306,-1658.9819,5.8906,91.1893,0,0, VEHICLE_RESPAWN, 1); // KING
	SAPDVehicles[38] = AddStaticVehicleEx(490,1544.5461,-1663.0781,5.8906,90.1802,0,0, VEHICLE_RESPAWN, 1); // KING
	SAPDVehicles[39] = AddStaticVehicleEx(490,1544.2188,-1667.8118,5.8906,86.7270,0,0, VEHICLE_RESPAWN, 1); // KING
	SAPDVehicles[40] = AddStaticVehicleEx(579,1544.0065,-1672.1172,5.8906,85.7179,1,1, VEHICLE_RESPAWN, 1); // Detectivve Huntley
	SAPDVehicles[41] = AddStaticVehicleEx(570,1543.4154,-1676.1980,5.8906,80.3220,1,1, VEHICLE_RESPAWN, 1); // Detective Huntley
	SAPDVehicles[42] = AddStaticVehicleEx(426,1544.4579,-1680.1866,5.8906,90.2734,83,83, VEHICLE_RESPAWN, 1); // Detective Premier


	for(new x;x<sizeof(SAPDVehicles);x++)
	{
	    format(strings, sizeof(strings), "1-ADAM-%d", SAPDVehicles[x]);
	...
	SAGSVehicles[0] = AddStaticVehicleEx(405, 1555.3026, -1774.7996, 13.2771, 90.3600, 0, 0, VEHICLE_RESPAWN); // Sentinel SAGS
	// SAGSVehicles[1] = AddStaticVehicleEx(405, 1455.3049, -1748.5181, 13.3789, 0.0000, 0, 0, VEHICLE_RESPAWN);
	SAGSVehicles[2] = AddStaticVehicleEx(409, 1554.2104, -1780.4286, 13.1390, 90.3000, 0, 0, VEHICLE_RESPAWN); // Stretch SAGS
	...
	SAGSVehicles[6] = AddStaticVehicleEx(411, 1555.4640, -1785.8137, 13.1743, 91.0800, 0, 0, VEHICLE_RESPAWN); // Infernus SAGS
	SAGSVehicles[7] = AddStaticVehicleEx(521, 1539.6965, -1756.3882, 12.9942, -180.3000, 0, 0, VEHICLE_RESPAWN); // FCR SAGS 2
	SAGSVehicles[8] = AddStaticVehicleEx(521, 1545.5199, -1756.4872, 12.9942, -180.3000, 0, 0, VEHICLE_RESPAWN); // FCR SAGS
	// SAGSVehicles[9] = AddStaticVehicleEx(521, 1515.9736, -1845.9476, 13.0548, 0.0000, 0, 0, VEHICLE_RESPAWN);
	SAGSVehicles[10] = AddStaticVehicleEx(437, 1534.1698, -1758.5048, 13.5445, -181.9798, 0, 0, VEHICLE_RESPAWN); // Bus SAGS
...
public OnPlayerText(playerid, text[])
{
    ...
    new msg[256]; 
    format(msg, sizeof(msg), "```[CHAT] %s(ID: %d): %s```", name, playerid, text);
    DCC_SendChannelMessage(g_Discord_Chat, msg);

	// While admin/helper on duty
	if(pData[playerid][pAdminDuty] == 1)
	{
		new String[256];
		format(String, sizeof(String), ""RED_E"%s: "WHITE_E"(( %s ))", pData[playerid][pAdminname], text);
		ProxDetector(20.0, playerid, String, COLOR_WHITE, COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE5);
		return 0;
	}
...
public OnPlayerCommandPerformed(playerid, cmd[], params[], result, flags)
{
    if (result == -1)
    {
        Error(playerid, "Unknown command, see '/help'");
        return 0;
    }
    ...
    GetPlayerName(playerid, name, sizeof name);
    new msg[256];
	format(msg, sizeof(msg), "```[CMD]: %s(ID: %d) telah menggunakan commands '/%s' (%s)```", name, playerid, cmd, params);
    DCC_SendChannelMessage(g_Discord_Chat, msg);
    return 1;
}
...
public OnPlayerConnect(playerid)
{
	...
	foreach(new ii : Player)
	{
		...
		{
			SendClientMessageEx(ii, X11_LIGHTBLUE, "SERVER: "RED"%s "YELLOW"Has been connected to the server. (%s, %s)", pData[playerid][pName], city, country);
		}
	}
    ...
    new string[128];
    format(string, sizeof string, " ```[LOG-IN] %s(ID: %d) telah terkoneksi kedalam server.```", name, playerid);
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	foreach(new ii : Player)
	{
		if(IsPlayerInRangeOfPoint(ii, 40.0, x, y, z))
		{
			switch(reason)
			{
				case 0:
				{
					SendClientMessageEx(ii, X11_LIGHTBLUE, "SERVER: "RED" %s "YELLOW"Has been disconnected from the server. (Disconnected/Crash)", pData[playerid][pName]);
				}
				case 1:
				{
					SendClientMessageEx(ii, X11_LIGHTBLUE, "SERVER: "RED" %s "YELLOW"Has been disconnected from the server. (Leave)", pData[playerid][pName]);
				}
				case 2:
				{
					SendClientMessageEx(ii, X11_LIGHTBLUE, "SERVER: "RED" %s "YELLOW"Has been disconnected from the server. (Kicked/Banned)", pData[playerid][pName]);
				}
			}
		}
	}
	// Disconnect message send to discord
	new name[MAX_PLAYER_NAME + 1];
    GetPlayerName(playerid, name, sizeof name);
    ...
    new string[128];
    format(string, sizeof string, " ```[LOG-OUT] %s(ID: %d) telah keluar dari server.```", name, playerid);
    DCC_SendChannelMessage(g_Discord_Chat, string);
	return 1;
}
...
public OnPlayerSpawn(playerid)
{
	...
	PlayerPlaySound(playerid, 1186, 0.0, 0.0, 0.0); // Stop login music
	TogglePlayerSpectating(playerid, 0); // Stop Spectating while Interpolating Login
...
public OnPlayerEditDynamicObject(playerid, STREAMER_TAG_OBJECT: objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
	...
	if (EditingObject[playerid] == 1 && Iter_Contains(Obj, EditingObjectID[playerid])) {
		new id = EditingObjectID[playerid];
		if (response == EDIT_RESPONSE_UPDATE) {
			SetDynamicObjectPos(objectid, x, y, z);
			SetDynamicObjectRot(objectid, rx, ry, rz);
		} else if (response == EDIT_RESPONSE_CANCEL) {
			SetDynamicObjectPos(objectid, objPosX[playerid], objPosY[playerid], objPosZ[playerid]);
			SetDynamicObjectRot(objectid, objRotX[playerid], objRotY[playerid], objRotZ[playerid]);
			objPosX[playerid] = 0; objPosY[playerid] = 0; objPosZ[playerid] = 0;
			objRotX[playerid] = 0; objRotY[playerid] = 0; objRotZ[playerid] = 0;
			Servers(playerid, " You have canceled editing object ID %d.", id);
			EditingObject[playerid] = 0; EditingObjectID[playerid] = -1;
			Obj_Save(id);
		} else if (response == EDIT_RESPONSE_FINAL) {
			ObjData[id][objPos][0] = x;
			ObjData[id][objPos][1] = y;
			ObjData[id][objPos][2] = z;
			ObjData[id][objRPos][0] = rx;
			ObjData[id][objRPos][1] = ry;
			ObjData[id][objRPos][2] = rz;

			SetDynamicObjectPos(objectid, ObjData[id][objPos][0], ObjData[id][objPos][1], ObjData[id][objPos][2]);
			SetDynamicObjectRot(objectid, ObjData[id][objRPos][0], ObjData[id][objRPos][1], ObjData[id][objRPos][2]);
			EditingObject[playerid] = 0; EditingObjectID[playerid] = -1;
			Servers(playerid, " You have finishing edit object id: %d.", id);
			Obj_Save(id);
		}
	}
	return 1;
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	// Vehicle Lock Shortcut by AzharHrj
	...
	if(VoteOn && VoteVoted[playerid] == 0)
	{
	    if(newkeys == KEY_YES)
	    {
	        ...
	        SendClientMessage(playerid, COLOR_LIGHTGREEN, "[VOTE]{FFFFFF} Anda memilih {33AA33}IYA{FFFFFF}.");
		}
	    if(newkeys == KEY_NO)
	    {
		    VoteN++;
		    VoteVoted[playerid] = 1;
		    SendClientMessage(playerid, COLOR_LIGHTGREEN, "[VOTE]{FFFFFF} Anda memilih {FF0000}TIDAK{FFFFFF}.");
	    }
	}
	//Anti BH by azharhrj
	if((newkeys & KEY_JUMP) && !IsPlayerInAnyVehicle(playerid))
    {
		...
		//SetTimerEx("ResetJump", 3000, false, "i", playerid);
        ...
        if(PlayerPressedJump[playerid] == 3)
        {
            if(pData[playerid][pAdmin] > 1) {
            	...
            	SetTimerEx("PressJump", 5000, false, "i", playerid);
			}
        }
    }
	return 1;
...
{
	if(pData[playerid][pAdmin] >= 1)
	//if(pData[playerid][pAdminDuty] = 1);// Bug
	if(pData[playerid][pAdminDuty] == 1)
	{
...
{
    PlayerPressedJump[playerid] = 0;
    ApplyAnimation(playerid, "PED", "GETUP_FRONT", 4.1, 0, 0, 0, 0, 0, 0);
    return 1;
}
