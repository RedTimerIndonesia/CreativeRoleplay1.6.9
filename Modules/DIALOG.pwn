...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_LOGIN)
    {
		...
		else
		{
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Login", "You have mistyped your password too often (5 times).", "Okay", "");
				KickEx(playerid);
			}
			else {
				// ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login", "Wrong password!\nPlease enter your password in the field below:", "Login", "Abort");
				format(string, sizeof string, ""LB_E"Username: "WHITE_E"%s\n"LB_E"Password: "GREEN_E"(input below)\n"LB_E"Attemps: "WHITE_E"%d / 5", pData[playerid][pName], pData[playerid][LoginAttempts]);
				ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "RT:RP - Login", string, "Login", "Quit");
			}
		}
        return 1;
    }
	    ...
	    if(response)
	    {
	        ...
	        if(listitem == 5)
	        {
	            if(ServerLocked == true)
	            {
	                ...
	                format(string, sizeof(string), "%s has unlocked the server", pData[playerid][pAdminname]);
	...
	if(dialogid == LOCK_SERVER)
	{
	    ...
	    if(response)
	    {
			if(ServerLocked == false)
			{
			    ...
			    SendRconCommand(string);
			    format(string, sizeof(string), "%s has locked the server", pData[playerid][pAdminname]);
       			SendClientMessageToAll(COLOR_LRED, string);
			}
	...
	if(dialogid == CHANGE_HOSTNAME)
	{
	    ...
	    if(response)
	    {
	        ...
	        SendRconCommand(string);
	        format(string, sizeof(string), "%s has change server hostname", pData[playerid][pAdminname]);
         	SendClientMessageToAll(COLOR_LRED, string);
	    }
		return 1;
	}
	if(dialogid == MAPNAME_DIALOG)
	{
	    ...
	    if(response)
	    {
	        ...
	        SendRconCommand(string);
	        format(string, sizeof(string), "%s has change server mapname", pData[playerid][pAdminname]);
         	SendClientMessageToAll(COLOR_LRED, string);
	    }
	    return 1;
	}
	if(dialogid == WEBURL_DIALOG)
	{
	    ...
	    if(response)
	    {
	        ...
	        SendRconCommand(string);
	        format(string, sizeof(string), "%s has change server weburl", pData[playerid][pAdminname]);
         	SendClientMessageToAll(COLOR_LRED, string);
	    }
	    return 1;
	}
	if(dialogid == LANGUAGE_DIALOG)
	{
	    ...
	    if(response)
	    {
	        ...
	        SendRconCommand(string);
	        format(string, sizeof(string), "%s has change server language", pData[playerid][pAdminname]);
         	SendClientMessageToAll(COLOR_LRED, string);
	    }
	    return 1;
	}
	...
	if(dialogid == DIALOG_MY_BISNIS)
	{
		...
		ShowPlayerDialog(playerid, BISNIS_INFO, DIALOG_STYLE_LIST, "{FF0000}RT:RP {0000FF}Bisnis", "Show Information\nTrack Bisnis", "Select", "Cancel");
		return 1;
	}
	...
	if(dialogid == DIALOG_MY_HOUSES)
	{
		...
		ShowPlayerDialog(playerid, HOUSE_INFO, DIALOG_STYLE_LIST, "{FF0000}RT:RP {0000FF}Houses", "Show Information\nTrack House", "Select", "Cancel");
		return 1;
	}
			...
			switch(listitem)
			{
				case 0: //slot 1
				{
					...
					else
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"RT:RP "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
				}
				case 1: //slot 2
				{
					...
					else
					{
						...
						pToys[playerid][pData[playerid][toySelected]][toy_rx], pToys[playerid][pData[playerid][toySelected]][toy_ry], pToys[playerid][pData[playerid][toySelected]][toy_rz]);
						ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"RT:RP "WHITE_E"Player Toys", string, "Select", "Cancel");
						//ShowPlayerDialog(playerid, DIALOG_TOYEDIT, DIALOG_STYLE_LIST, ""RED_E"RT:RP "WHITE_E"Player Toys", ""dot"Edit Toy Position\n"dot"Change Bone\n"dot""GREY_E"Remove Toy\n"dot"Share Toy Pos", "Select", "Cancel");
					}
				}
				case 2: //slot 3
				{
		...
		if(response)
		{
			switch (listitem) 
			{
				...
				case 3:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYVIP, DIALOG_STYLE_LIST, ""RED_E"RT:RP "WHITE_E"VIP Toys", string, "Select", "Cancel");
				}
			}
		}
	}
	...
	if(dialogid == DIALOG_ARMS_GUN)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 6: //Micro smg uzi == Change to MP5
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
					if(pData[playerid][pMaterial] < 500) return Error(playerid, "Material tidak cukup!(Butuh: 500).");
					if(pData[playerid][pComponent] < 500) return Error(playerid, "Component tidak cukup!(Butuh: 500).");
					...
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 1000, true, "idd", playerid, WEAPON_MP5, 200);
					PlayerTextDrawSetString(playerid, ActiveTD[playerid], "Creating...");
					PlayerTextDrawShow(playerid, ActiveTD[playerid]);
					ShowPlayerProgressBar(playerid, pData[playerid][activitybar]);
				}
