...
function LoadPlayerVehicle(playerid)
{
	...
	if(count > 0)
	{
		for(new z = 0; z < count; z++)
		{
			...
			cache_get_value_float(z, "x", pvData[i][cPosX]);
			cache_get_value_float(z, "y", pvData[i][cPosY]);
			cache_get_value_float(z, "z", pvData[i][cPosZ]);
			cache_get_value_float(z, "a", pvData[i][cPosA]);
			...
			cache_get_value_int(z, "damage0", pvData[i][cDamage0]);
			cache_get_value_int(z, "damage1", pvData[i][cDamage1]);
			cache_get_value_int(z, "damage2", pvData[i][cDamage2]);
			cache_get_value_int(z, "damage3", pvData[i][cDamage3]);
			...
			cache_get_value_int(z, "neon", pvData[i][cNeon]);
			pvData[i][cTogNeon] = 0;
			...
			cache_get_value_int(z, "gasoil", pvData[i][cGasOil]);
			cache_get_value_int(z, "rental", pvData[i][cRent]);
			/*for(new x = 0; x < 17; x++)
			{
...
function RemovePlayerVehicle(playerid)
{
	foreach(new i : PVehicles)
	{
		...
		if(pvData[i][cOwner] == pData[playerid][pID])
		{
			...
			mysql_format(g_SQL, cQuery, sizeof(cQuery), "%s`z`='%f', ", cQuery, pvData[i][cPosZ]);
...
function OnVehRentPV(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a, rental)
{
	...
	Iter_Add(PVehicles, i);
	OnPlayerVehicleRespawn(i);
	Servers(playerid, "Anda telah menyewa kendaraan seharga $500 / 3 days dengan model %s(%d)", GetVehicleModelName(model), model);
	SetPlayerPosition(playerid, 1802.51, -1797.49, 13.54, 1.28, 0);
	pData[playerid][pBuyPvModel] = 0;
	return 1;
}
...
CMD:v(playerid, params[])
{
	...
	{
		new bool:found = false, msg2[512], Float:fx, Float:fy, Float:fz;
		format(msg2, sizeof(msg2), "Model\tInsurance\tClaim\n");
		foreach(new i : PVehicles)
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
			{
				GetVehiclePos(pvData[i][cVeh], fx, fy, fz);
				format(msg2, sizeof(msg2), "%s\t%s(id: %d)\t%d\t%d\n", msg2, GetVehicleModelName(pvData[i][cModel]), pvData[i][cVeh], pvData[i][cInsu], pvData[i][cClaim]);
				found = true;
			}
		}
		if(found)
