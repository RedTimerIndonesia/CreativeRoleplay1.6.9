...
{	
	...
	foreach(new ii : Player)
	{
		...
		if(pData[ii][pJail] > 0)
		{
			if(pData[ii][pJailTime] > 0)
			{
				pData[ii][pJailTime]--;
				new mstr[128];
				format(mstr, sizeof(mstr), "~b~~h~You will be unjail in ~w~%d ~b~~h~seconds.", pData[ii][pJailTime]);
				InfoTD_MSG(ii, 1000, mstr);
			}
			else
			{
				pData[ii][pJail] = 0;
				pData[ii][pJailTime] = 0;
				//SpawnPlayer(ii);
				...
				SendClientMessageToAllEx(COLOR_RED, "Server: "GREY2_E" %s(%d) have been un-jailed by the server. (times up)", pData[ii][pName], ii);
			}
		}
		//Arreset Player
		if(pData[ii][pArrest] > 0)
		{
