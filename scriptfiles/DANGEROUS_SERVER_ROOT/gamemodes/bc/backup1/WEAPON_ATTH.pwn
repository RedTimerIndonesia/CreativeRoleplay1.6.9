...
CMD:weapon(playerid, params[])
{
	...
	else if(!strcmp(name, "give", true))
	{
		...
		Info(playerid, "Anda telah memberikan weapon %s kepada %s.", ReturnWeaponName(weaponid) , pData[playerid][pName]);
		Info(otherid, "%s telah memberikan weapon %s kepada anda.", ReturnWeaponName(weaponid) , pData[playerid][pName]);
	}
	...
	{
		new id = NearWeapon(playerid);
		if(id != -1)
        {
			PickupWeapon(playerid, id);
			SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, "* %s has picked up a %s.", ReturnName(playerid), ReturnWeaponName(DropWeap[id][WeaponID]));
			ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 4.1, 0, 0, 0, 0, 0, 1);
		}
	}
