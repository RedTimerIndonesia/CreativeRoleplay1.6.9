...
CMD:factionhelp(playerid)
{
	if(pData[playerid][pFaction] == 1)
	{
		...
		strcat(str, ""WHITE_E"SAPD: /sapdonline /(un)cuff /tazer /detain /arrest /release\n");
	...
	else if(pData[playerid][pFaction] == 3)
	{
		...
		strcat(str, ""WHITE_E"SAMD: /samdonline /loadinjured /dropinjured\n");
		strcat(str, ""WHITE_E"NOTE: Lama waktu duty anda akan menjadi gaji anda pada saat paycheck!\n");
	...
	{
		new str[3500];
		strcat(str, ""WHITE_E"SANA: /fsave /f(amily) /finvite /funinvite\n");
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""RED_E"SANA", str, "Close", "");
	}
	else
	{
...
CMD:d(playerid, params[])
{
    ...
    if(pData[playerid][pFaction] == 1) {
        ...
        SendFactionMessage(4, 0xFFD7004A, "** [SANA Departement] %s(%d) %s: %s", GetFactionRank(playerid),  pData[playerid][pFactionRank], pData[playerid][pName], text);
		format(mstr, sizeof(mstr), "[<DEPARTEMENT>]\n* %s *", text);
		SetPlayerChatBubble(playerid, mstr, 0xFFD7004A, 10.0, 3000);
    }
    else
...
CMD:release(playerid, params[])
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1526.69, -1678.05, 5.89))
