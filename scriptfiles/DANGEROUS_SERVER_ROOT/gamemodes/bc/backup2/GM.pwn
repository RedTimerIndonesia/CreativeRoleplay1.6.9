...
public OnGameModeInit()
{
	...
	format(strings, sizeof(strings), "[License]\n{FFFFFF}/newdrivelic to create new license");
	CreateDynamic3DTextLabel(strings, COLOR_LBLUE, 252.22, 117.43, 1003.21, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // Driving Lic
	
	CreateDynamicPickup(1239, 23, 240.80, 112.95, 1003.21, -1);
	format(strings, sizeof(strings), "[Plate]\n{FFFFFF}/buyplate to create new plate");
	CreateDynamic3DTextLabel(strings, COLOR_LBLUE, 240.80, 112.95, 1003.21, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // Plate
	
	CreateDynamicPickup(1239, 23, 224.11, 118.50, 999.10, -1);
	format(strings, sizeof(strings), "[ARREST POINT]\n{FFFFFF}/arrest to arrest wanted player");
	CreateDynamic3DTextLabel(strings, COLOR_LBLUE, 224.11, 118.50, 999.10, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // arrest
	...
	format(strings, sizeof(strings), "[BANK]\n{FFFFFF}/newrek to create new rekening");
	...
	SANAVehicles[0] = AddStaticVehicleEx(582, 758.7664, -1336.1642, 13.9482, 179.0212, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(582, 764.4276, -1336.1959, 13.9482, 179.0212, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(582, 770.3247, -1335.9663, 13.9482, 179.0212, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(582, 781.8044, -1348.0001, 13.9482, 91.0000, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(582, 781.6168, -1342.6869, 13.9482, 91.0000, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(418, 737.3025, -1334.3344, 14.1711, 246.6513, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(413, 736.4621, -1338.6304, 13.7490, -113.0000, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(405, 737.4107, -1343.0820, 13.7357, -113.0000, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(461, 749.7194, -1334.2122, 13.2465, 178.0000, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(461, 753.8127, -1334.2727, 13.2465, 178.0000, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(461, 751.7328, -1334.2236, 13.2465, 178.0000, -1, -1, VEHICLE_RESPAWN);
	SANAVehicles[0] = AddStaticVehicleEx(488, 741.9925, -1371.2443, 25.8111, 0.0000, -1, -1, VEHICLE_RESPAWN);
...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessageToAllEx(COLOR_RED, "[SERVER]"YELLOW_E" %s(%d) has joined to the server.(%s, %s)", pData[playerid][pName], playerid, city, country);
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	switch(reason)
	{
	    case 0:
		{
			SendClientMessageToAllEx(COLOR_RED, "[SERVER]"YELLOW_E" %s(%d) has leave from the server.(timeout/crash)", pData[playerid][pName], playerid);
		}
	    case 1:
		{
			SendClientMessageToAllEx(COLOR_RED, "[SERVER]"YELLOW_E" %s(%d) has leave from the server.(leaving)", pData[playerid][pName], playerid);
		}
	    case 2:
		{
	 		SendClientMessageToAllEx(COLOR_RED, "[SERVER]"YELLOW_E" %s(%d) has leave from the server.(kicked/banned)", pData[playerid][pName], playerid);
		}
	}
	return 1;
}
...
SetPlayerSpawn(playerid)
{
	if(IsPlayerConnected(playerid))
	{
		if(pData[playerid][pGender] == 0)
		{
			...
			ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, "Tanggal Lahir", "Masukan tanggal lahir (Tgl/Bulan/Tahun):", "Pilih", "Batal");
		}
		else
		{
...
public OnPlayerEditDynamicObject(playerid, STREAMER_TAG_OBJECT: objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
	...
	else if(pData[playerid][EditingOreID] != -1 && Iter_Contains(Ores, pData[playerid][EditingOreID]))
	{
	    if(response == EDIT_RESPONSE_FINAL)
	    {
	...
	if(pData[playerid][pSideJob] == 1)
	{
		...
		if(GetVehicleModel(vehicleid) == 574)
		{
			...
			if(IsPlayerInRangeOfPoint(playerid, 7.0,sweperpoint12))
			{
				...
				pData[playerid][pSideJobTime] = 500;
				DisablePlayerCheckpoint(playerid);
				AddPlayerSalary(playerid, "Sidejob(Sweeper)", 700);
...
ptask PlayerUpdate[999](playerid)
{
	...
	if(pData[playerid][pSpawned] == 1)
    {
        if(GetPlayerWeapon(playerid) != pData[playerid][pWeapon])
        {
            ...
            {
                SendAdminMessage(COLOR_RED, "%s has possibly used weapon hacks (%s), Please to check /spec this player first!", pData[playerid][pName], ReturnWeaponName(pData[playerid][pWeapon]));
