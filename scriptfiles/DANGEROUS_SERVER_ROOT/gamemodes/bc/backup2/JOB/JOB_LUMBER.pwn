CreateJoinLumberPoint()
{
	//JOBS
	new strings[128];
	CreateDynamicPickup(1239, 23, -265.87, -2213.63, 29.04, -1);
...
CMD:lumber(playerid, params[])
{
	if(pData[playerid][pJob] == 3 || pData[playerid][pJob2] == 3)
	{
		...
		else if(!strcmp(params, "load", true))
		{
			...
			if(GetVehicleModel(vehicleid) != 422) return Error(playerid, "You're not near a Bobcat.");
		...
		else if(!strcmp(params, "unload")) 
		{
			...
			if(Vehicle_LumberCount(vehicleid) < 1) return Error(playerid, "This Bobcat doesn't have any logs.");
