...
Player_GiveLog(playerid, type)
{
    ...
    pData[playerid][LoadingPoint] = CreateDynamicCP(x, y, z, 3.0, .playerid = playerid);
	
	Info(playerid, "You can press "GREEN_E"~k~~CONVERSATION_NO~ "WHITE_E"to drop your log.");
	return 1;
}
...
function RespawnOre(id)
{
	...
	{
	    KillTimer(OreData[id][oreTimer]);

	    OreData[id][oreLog] = 5;
	    OreData[id][oreSeconds] = 0;
	    OreData[id][oreTimer] = -1;
 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_int(i, "type", OreData[tid][oreType]);
			cache_get_value_float(i, "posx", OreData[tid][oreX]);
			cache_get_value_float(i, "posy", OreData[tid][oreY]);
			cache_get_value_float(i, "posz", OreData[tid][oreZ]);
			cache_get_value_float(i, "posrx", OreData[tid][oreRX]);
			cache_get_value_float(i, "posry", OreData[tid][oreRY]);
			cache_get_value_float(i, "posrz", OreData[tid][oreRZ]);
...
Ore_Refresh(tid)
{
    ...
    if(OreData[tid][oreLog] > 0) {
	    format(label, sizeof(label), "Ore (%d - %s)\n"WHITE_E"Ore: "GREEN_E"%d\n"WHITE_E"Use /ore mine.", tid, type, OreData[tid][oreLog]);
		UpdateDynamic3DTextLabelText(OreData[tid][oreLabel], COLOR_GREEN, label);
	}else{
	    OreData[tid][oreTimer] = SetTimerEx("RespawnOre", 1000, true, "i", tid);
	    
	    format(label, sizeof(label), "Ore (%d - %s)\n"WHITE_E"%s", tid, type, ConvertToMinutes(OreData[tid][oreSeconds]));
		UpdateDynamic3DTextLabelText(OreData[tid][oreLabel], COLOR_GREEN, label);
	}
	return 1;
}
...
CMD:removeore(playerid, params[])
{
	...
	if(OreData[tid][oreTimer] != -1) KillTimer(OreData[tid][oreTimer]);
	
	OreData[tid][oreLog] = OreData[tid][oreSeconds] = 0;
	OreData[tid][oreObjID] = OreData[tid][oreTimer] = -1;
...
CMD:ore(playerid, params[])
{
	...
	{
		if(isnull(params)) return Usage(playerid, "/ore [info/mine/pickup]");
		...
		if(!strcmp(params, "mine", true))
		{
			...
			if(!Vehicle_IsMiningVehicle(GetPVarInt(playerid, "LastVehicleID"))) return Error(playerid, "Your last vehicle isn't available for mining.");
		...
		else if(!strcmp(params, "info", true))
		{
			...
			if(!Vehicle_IsMiningVehicle(vehicleid)) return Error(playerid, "Vehicle isn't a mining vehicle.");
		...
		else if(!strcmp(params, "sell", true))
		{
			...
			Component += LogStorage[vehicleid][0];
			GasOil += LogStorage[vehicleid][1];
