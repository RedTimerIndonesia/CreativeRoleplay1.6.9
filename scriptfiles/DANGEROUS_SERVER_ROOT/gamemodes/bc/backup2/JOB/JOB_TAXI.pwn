...
CMD:taxiduty(playerid, params[])
{
	if(pData[playerid][pJob] == 1 || pData[playerid][pJob2] == 1)
	{
		...
		if(modelid != 438 && modelid != 420)
			return Error(playerid, "You must be inside a taxi.");
			
		if(pData[playerid][pTaxiDuty] == 0)
		{
			pData[playerid][pTaxiDuty] = 1;
			SetPlayerColor(playerid, COLOR_YELLOW);
			SendClientMessageToAllEx(COLOR_YELLOW, "[TAXI]"WHITE_E" %s is now on taxi duty. Type /call 1222 to call a taxi!", ReturnName(playerid));
		}
		else
		{
