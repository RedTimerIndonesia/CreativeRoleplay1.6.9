...
CMD:deletepv(playerid, params[])
{
	...
	foreach(new i : PVehicles)			
	{
		...
		else return Error(playerid, "This vehicle is not private vehicles.");
	}
	return 1;
}
...
CMD:aveh(playerid, params[])
{
	...
	Servers(playerid, "Vehicle ID near on you: %d (Model: %s(%d))", vehicleid, GetVehicleName(vehicleid), GetVehicleModel(vehicleid));
	return 1;
}
