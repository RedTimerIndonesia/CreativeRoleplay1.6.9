...
CMD:creategun(playerid, params[])
{
	...
	if(pData[playerid][pLevel] < 5) return Error(playerid, "You dont have skill to creategun.");
	
	new Dstring[512];
	format(Dstring, sizeof(Dstring), "Weapon\tMats & Comps\n\
	Silenced Pistol(ammo 70)\t200 & 200\n");
	format(Dstring, sizeof(Dstring), "%sDesert Eagle(ammo 70)\t400 & 400\n", Dstring);
	format(Dstring, sizeof(Dstring), "%sShotgun(ammo 30)\t300 & 300\n", Dstring);
	format(Dstring, sizeof(Dstring), "%sRifle(ammo 30)\t300 & 300\n", Dstring);
	format(Dstring, sizeof(Dstring), "%sAK-47(ammo 100)\t400 & 400\n", Dstring);
