...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
        ...
        if(strcmp(bData[id][bOwner], "-"))
		{
			...
			bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 4);
        }
        else
        {
		...
		bData[id][bCP] = CreateDynamicCP(bData[id][bIntposX], bData[id][bIntposY], bData[id][bIntposZ], 1, id, bData[id][bInt], -1, 3, -1, 0);
 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_float(i, "extposx", bData[bid][bExtposX]);
			cache_get_value_float(i, "extposy", bData[bid][bExtposY]);
			cache_get_value_float(i, "extposz", bData[bid][bExtposZ]);
			cache_get_value_float(i, "extposa", bData[bid][bExtposA]);
			cache_get_value_float(i, "intposx", bData[bid][bIntposX]);
			cache_get_value_float(i, "intposy", bData[bid][bIntposY]);
			cache_get_value_float(i, "intposz", bData[bid][bIntposZ]);
			cache_get_value_float(i, "intposa", bData[bid][bIntposA]);
			...
			cache_get_value_float(i, "pointx", bData[bid][bPointX]);
			cache_get_value_float(i, "pointy", bData[bid][bPointY]);
			cache_get_value_float(i, "pointz", bData[bid][bPointZ]);
