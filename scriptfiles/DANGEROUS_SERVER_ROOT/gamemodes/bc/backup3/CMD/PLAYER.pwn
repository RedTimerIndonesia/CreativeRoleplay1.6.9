...
CMD:donate(playerid)
{
	...
	"YELLOW_E"1) "WHITE_E"Gratis "LB_E"20 "WHITE_E"VIP Coin.\n");
	...
	"YELLOW_E"1) "WHITE_E"Gratis "LB_E"30"WHITE_E" VIP Coin.\n\
	...
	"YELLOW_E"1) "WHITE_E"Gratis "LB_E"40 "WHITE_E"VIP Coin.\n\
	"YELLOW_E"2) "WHITE_E"Mendapat "GREEN_E"2 "WHITE_E"slot job.\n\
...
{
	new Dstring[512];
	format(Dstring, sizeof(Dstring), "Coin Shop\tPrice\n\
	Instant Change Name\t30 Coin\n");
	format(Dstring, sizeof(Dstring), "%sClear Warning\t20 Coin\n", Dstring);
	format(Dstring, sizeof(Dstring), "%sVIP Level 1(7 Days)\t10 Coin\n", Dstring);
	format(Dstring, sizeof(Dstring), "%sVIP Level 2(7 Days)\t20 Coin\n", Dstring);
	format(Dstring, sizeof(Dstring), "%sVIP Level 3(7 Days)\t30 Coin\n", Dstring);
	format(Dstring, sizeof(Dstring), "%sConvert $100,000\t50 Coin\n", Dstring);
	ShowPlayerDialog(playerid, DIALOG_COINSHOP, DIALOG_STYLE_TABLIST_HEADERS, "Coin Shop", Dstring, "Buy", "Cancel");
	return 1;
}
	...
	if(IsPlayerConnected(playerid)) 
	{
		...
		else if(strcmp(params,"job",true) == 0) 
		{
			if(pData[playerid][pGetJob] > 0)
			{
				...
				pData[playerid][pExitJob] = gettime() + (3 * 86400);
			}
...
CMD:deny(playerid, params[])
{
	if(IsPlayerConnected(playerid)) 
	{
		...
		else if(strcmp(params,"job1",true) == 0) 
		{
			...
			if(pData[playerid][pExitJob] != 0) return Error(playerid, "You must wait 3 days for exit from your current job!");
			if(pData[playerid][pJob] != 0)
			{
...
CMD:ads(playerid, params[])
{
	...
	if(isnull(params))
	{
		Usage(playerid, "/ads [text] | 1 character pay $10");
		return 1;
	}
	if(strlen(params) >= 50 ) return Error(playerid, "Maximum character is 50 text." );
