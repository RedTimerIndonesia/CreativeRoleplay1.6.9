 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			cache_get_value_int(i, "id", gsid);
			cache_get_value_int(i, "stock", gsData[gsid][gsStock]);
			cache_get_value_float(i, "posx", gsData[gsid][gsPosX]);
			cache_get_value_float(i, "posy", gsData[gsid][gsPosY]);
			cache_get_value_float(i, "posz", gsData[gsid][gsPosZ]);
			GStation_Refresh(gsid);
...
function Filling(playerid)
{
	...
	foreach(new gsid : GStation)
	{
		...
		else
		{
			...
			SetVehicleFuel(GetPlayerVehicleID(playerid), old + 100);
			pData[playerid][pFillPrice] += GStationPrice;
			gsData[pData[playerid][pFill]][gsStock]--;
