...
House_Refresh(houseid)
{
    if(houseid != -1)
    {
        ...
        if(strcmp(hData[houseid][hOwner], "-"))
		{
			...
			hData[houseid][hPickup] = CreateDynamicPickup(1273, 23, hData[houseid][hExtposX], hData[houseid][hExtposY], hData[houseid][hExtposZ]+0.2, 0, 0, _, 4);
        }
        else
        {
		...
		hData[houseid][hCP] = CreateDynamicCP(hData[houseid][hIntposX], hData[houseid][hIntposY], hData[houseid][hIntposZ], 1, houseid, hData[houseid][hInt], -1, 3, -1, 0);
