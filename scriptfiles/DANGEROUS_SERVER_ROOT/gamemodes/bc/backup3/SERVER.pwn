...
CreateServerPoint()
{
	...
	FoodPickup = CreateDynamicPickup(1239, 23, -381.44, -1426.13, 25.93, -1);
	format(strings, sizeof(strings), "[Food]\n"WHITE_E"Food Stock: "LG_E"%d\n\n"WHITE_E"Food Price: "LG_E"%s / item\n\n"WHITE_E"Fish Price: "LG_E"%s / kg\n"LB_E"/sellfish", Food, FormatMoney(FoodPrice), FormatMoney(FishPrice));
	FoodText = CreateDynamic3DTextLabel(strings, COLOR_YELLOW, -381.44, -1426.13 , 25.93, 5.0); // food
}
...
Server_Save()
{
	...
	CreateServerPoint();
    format(str, sizeof(str), "UPDATE server SET servermoney='%d', material='%d', materialprice='%d', lumberprice='%d', component='%d', componentprice='%d', metalprice='%d', gasoil='%d', gasoilprice='%d', coalprice='%d', product='%d', productprice='%d', medicineprice='%d', medkitprice='%d', food='%d', foodprice='%d', fishprice='%d', gstationprice='%d' WHERE id=0",
	ServerMoney, Material, MaterialPrice, LumberPrice, Component, ComponentPrice, MetalPrice, GasOil, GasOilPrice, CoalPrice, Product, ProductPrice, MedicinePrice, MedkitPrice, Food, FoodPrice, FishPrice, GStationPrice);
