 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			cache_get_value_int(i, "id", voucid);
			cache_get_value_int(i, "code", VoucData[voucid][voucCode]);
			cache_get_value_int(i, "vip", VoucData[voucid][voucVIP]);
			cache_get_value_int(i, "vip_time", VoucData[voucid][voucVIPTime]);
			cache_get_value_int(i, "coin", VoucData[voucid][voucCoin]);
			cache_get_value(i, "admin", admin);
			format(VoucData[voucid][voucAdmin], 16, admin);
			cache_get_value(i, "donature", donature);
			format(VoucData[voucid][voucDonature], 16, donature);
			cache_get_value_int(i, "claim", VoucData[voucid][voucClaim]);
...
{
	new cQuery[512];
	format(cQuery, sizeof(cQuery), "UPDATE vouchers SET code='%d', vip='%d', vip_time='%d', coin='%d', admin='%s', donature='%s', claim='%d' WHERE id='%d'",
	VoucData[voucid][voucCode],
	VoucData[voucid][voucVIP],
	VoucData[voucid][voucVIPTime],
	VoucData[voucid][voucCoin],
...
CMD:createvoucher(playerid, params[])
{
	...
	new code, vip, viptime, coin;
	if(sscanf(params, "dddd", code, vip, viptime, coin)) return Usage(playerid, "/createvoucher [CODE(73821)] [VIP] [VIP-TIME(Days)] [COIN]");
	...
	if(coin < 0 || coin > 50) return Error(playerid, "Invalid coin 0-50.");
	...
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO vouchers SET id='%d', code='%d', vip='%d', vip_time='%d', coin='%d', admin='%s'", voucid, VoucData[voucid][voucCode], VoucData[voucid][voucVIP], VoucData[voucid][voucVIPTime], VoucData[voucid][voucCoin], pData[playerid][pAdminname]);
	mysql_tquery(g_SQL, query, "OnVoucherCreated", "i", voucid);
	
	Servers(playerid, "Voucher created id: %d | code: %d | vip: %d | vip-time: %d | coin: %d | admin: %s", voucid, VoucData[voucid][voucCode], VoucData[voucid][voucVIP], VoucData[voucid][voucVIPTime], VoucData[voucid][voucCoin], pData[playerid][pAdminname]);
	return 1;
}
	...
	foreach(new vo : Vouchers)
	{
		if(VoucData[vo][voucCode] == code)
		{
			if(VoucData[vo][voucClaim] == 0)
			{
				...
				{
					pData[playerid][pCoin] += VoucData[vo][voucCoin];
					
					VoucData[vo][voucClaim] = 1;
					format(VoucData[vo][voucDonature], 16, pData[playerid][pName]);
					Voucher_Save(vo);
					
					Info(playerid, "Voucher claimed. coin: %d | claimby: %s.", VoucData[vo][voucCoin], pData[playerid][pName]);
				}
				else
				{
					...
					Info(playerid, "Voucher claimed. VIP: %d | VIP TIME: %d days | coin: %d | claimby: %s.", VoucData[vo][voucVIP], dayz, VoucData[vo][voucCoin], pData[playerid][pName]);
				}
			}
