...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_LOGIN)
    {
		...
		{
			new query[512];
			mysql_format(g_SQL, query, sizeof(query), "SELECT * FROM `players` WHERE `username` = '%e'", pData[playerid][pName]);
	...
	if(dialogid == DIALOG_GOLDSHOP)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					if(pData[playerid][pGold] < 30) return Error(playerid, "Not enough gold!");
					ShowPlayerDialog(playerid, DIALOG_GOLDNAME, DIALOG_STYLE_INPUT, "Change Name", "Input new nickname:\nExample: Charles_Sanders\n", "Change", "Cancel");
				}
				case 1:
				{
					if(pData[playerid][pGold] < 20) return Error(playerid, "Not enough gold!");
					pData[playerid][pGold] -= 20;
					pData[playerid][pWarn] = 0;
					Info(playerid, "Warning have been reseted for 20 gold. Total Warning: 0");
				}
				case 2:
				{
					if(pData[playerid][pGold] < 10) return Error(playerid, "Not enough gold!");
					pData[playerid][pGold] -= 10;
					pData[playerid][pVip] = 1;
					pData[playerid][pVipTime] = gettime() + (7 * 86400);
					Info(playerid, "You has bought VIP level 1 for 10 gold(7 days).");
				}
				case 3:
				{
					if(pData[playerid][pGold] < 20) return Error(playerid, "Not enough gold!");
					...
					pData[playerid][pVipTime] = gettime() + (7 * 86400);
					Info(playerid, "You has bought VIP level 2 for 20 gold(7 days).");
				}
				case 4:
				{
					if(pData[playerid][pGold] < 30) return Error(playerid, "Not enough gold!");
					...
					pData[playerid][pVipTime] = gettime() + (7 * 86400);
					Info(playerid, "You has bought VIP level 3 for 30 gold(7 days).");
				}
			}
		}
		return 1;
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 4:
				{
					...
					if(bData[bid][bMoney] < 10000)
						return Error(playerid, "Setidaknya anda mempunyai uang dalam bisnis anda senilai $10.000 untuk merestock product.");
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					new tstr[128];
					pData[playerid][pBuyPvModel] = modelid;
					format(tstr, sizeof(tstr), ""WHITE_E"Anda akan menyewa kendaraan "PINK_E"%s "WHITE_E"dengan harga "LG_E"$500 / 3 days", GetVehicleModelName(modelid));
	...
	if(dialogid == DIALOG_PAYCHECK)
	{
		if(response)
		{
			...
			if(rows) 
			{
				new list[2000], date[30], info[16], money, totalduty, gajiduty, totalsal, total, bunga, pajak, hasil;
				...
				{
					bunga = pData[playerid][pBankMoney] + 1000;
				}
				else if(pData[playerid][pVip] == 2)
				{
					bunga = pData[playerid][pBankMoney] + 1500;
				}
				else if(pData[playerid][pVip] == 3)
				{
					bunga = pData[playerid][pBankMoney] + 2000;
				}
				else
				{
					bunga = 0;
			...
			{
				new list[2000], totalduty, gajiduty, total, bunga, pajak, hasil;
