 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			cache_get_value_int(i, "id", lid);
			cache_get_value_int(i, "type", lData[lid][lType]);
			cache_get_value_float(i, "posx", lData[lid][lPosX]);
			cache_get_value_float(i, "posy", lData[lid][lPosY]);
			cache_get_value_float(i, "posz", lData[lid][lPosZ]);
			cache_get_value_int(i, "interior", lData[lid][lInt]);
			Locker_Refresh(lid);
...
CMD:createlocker(playerid, params[])
{
	...
	new type;
	if(sscanf(params, "d", type)) return Usage(playerid, "/createlocker [type, 1.SAPD 2.SAGS 3.SAMD 4.SANA]");
...
CMD:gotolocker(playerid, params[])
{
	...
	SendClientMessageEx(playerid, COLOR_WHITE, "You has teleport to house id %d", lid);
	return 1;
}
...
CMD:editlocker(playerid, params[])
{
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(COLOR_RED, "%s has adjusted the location of house ID: %d.", pData[playerid][pAdminname], lid);
    }
    else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", tipe))
            return Usage(playerid, "/edithouse [id] [type] [1-4]");
