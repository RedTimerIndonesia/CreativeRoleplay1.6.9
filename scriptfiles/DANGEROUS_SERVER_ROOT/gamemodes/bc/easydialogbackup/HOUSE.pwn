...
House_Refresh(houseid)
{
    if(houseid != -1)
    {
        ...
        {
            format(string, sizeof(string), "[ID: %d]\n{00FF00}This house for sell\n{FFFFFF}House Location: {FFFF00}%s\n{FFFFFF}House Type: {FFFF00}%s\n{FFFFFF}House Price: {FFFF00}%s\n"WHITE_E"Type /buy to purchase", houseid, GetLocation(hData[houseid][hExtpos][0], hData[houseid][hExtpos][1], hData[houseid][hExtpos][2]), FormatMoney(hData[houseid][hPrice]), type);
            hData[houseid][hPickup] = CreateDynamicPickup(1273, 23, hData[houseid][hExtpos][0], hData[houseid][hExtpos][1], hData[houseid][hExtpos][2]+0.2, 0, 0, _, 4);
        }
...
CMD:myhouse(playerid)
{
	...
	Dialog_Show(playerid, DIALOG_MY_HOUSES, DIALOG_STYLE_LIST, "{FF0000}XG:RP: {0000FF}Houses", CMDSString, "Select", "Cancel");
	return 1;
}
...
//--------------[ House Dialog ]----------
Dialog:DIALOG_MY_HOUSES(playerid, response, listitem, inputtext[])
{
	if(!response) return true;
	SetPVarInt(playerid, "ClickedHouse", ReturnPlayerHousesID(playerid, (listitem + 1)));
	Dialog_Show(playerid, DIALOG_HOUSE_INFO, DIALOG_STYLE_LIST, "{FF0000}XG:RP {0000FF}Houses", "Show Information\nTrack House", "Select", "Cancel");
	return 1;
}

Dialog:DIALOG_HOUSE_INFO(playerid, response, listitem, inputtext[])
{
	if(!response) return true;
...
Dialog:HouseMoney(playerid, response, listitem, inputtext[])
{
    ...
    if(response)
    {
        switch (listitem)
        {
            case 0: 
			{
                Dialog_Show(playerid, HouseWithdrawCash, DIALOG_STYLE_INPUT, "Withdraw from safe", "Safe Balance: %s\n\nPlease enter how much money you wish to withdraw from the safe:", "Withdraw", "Back", FormatMoney(hData[houseid][hMoney]));
            }
            case 1: 
			{
                Dialog_Show(playerid, HouseDepositCash, DIALOG_STYLE_INPUT, "Deposit into safe", "Safe Balance: %s\n\nPlease enter how much money you wish to deposit into the safe:", "Withdraw", "Back", FormatMoney(hData[houseid][hMoney]));
            }
        }
	}
