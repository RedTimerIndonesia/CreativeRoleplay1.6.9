...
Dialog:DIALOG_TRACK_CAR(playerid, response, listitem, inputtext[])
{
    if(response) 
	{
        ...
        GetVehiclePos(carid, posisi[0], posisi[1], posisi[2]);
...
CMD:getdoors(playerid, params[])
{
	...
	Info(playerid, "%d %d %d %d", bonnet, boot, driver_door, passenger_door);
	return 1;
}
...
CMD:v(playerid, params[])
{
	...
	else if(!strcmp(type,"track",true))
	{
		...
		if(found)
			...
			Dialog_Show(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Vehicles", "Anda tidak memeliki kendaraan", "Close", "");
	}
	return 1;
}