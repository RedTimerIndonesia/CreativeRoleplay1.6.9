...
Plant_Refresh(id)
{
	if(id != -1)
    {
		...
		if(PlantData[id][PlantType] == 1)
		{
			...
			else if(PlantData[id][PlantTime] < 2400 && PlantData[id][PlantTime] > 1)
			{
...
function PlantGrowup(id)
{
	if(id != -1)
	{
		...
		if(PlantData[id][PlantTime] == 2300)
		{
			Plant_Refresh(id);
		}
		if(PlantData[id][PlantTime] == 1)
		{
			Plant_Refresh(id);
		}
	}
 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_int(i, "type", PlantData[id][PlantType]);
			cache_get_value_int(i, "time", PlantData[id][PlantTime]);
			cache_get_value_float(i, "posx", PlantData[id][PlantX]);
			cache_get_value_float(i, "posy", PlantData[id][PlantY]);
			cache_get_value_float(i, "posz", PlantData[id][PlantZ]);
			Iter_Add(Plants, id);
